...
### 4.3 Git 全自动流程（已授权）
**双远端策略（方案 B）**
- `origin` 指向私有仓库，包含完整 `workspace/records` 历史。
- `public` 指向开源仓库，必须剔除 `workspace/records`（发布分支/过滤推送）。

**自动提交范围**
- 当仓库内**任一受控文件**有变更时（包含 `AGENTS.md` 与 `.codex/schema.json` 等）：
  - 自动执行：`git add` → `git commit` → `git push`（仅推送到 `origin`）
  - commit message 自动生成（简洁、可读）
- **禁止自动推送到 `public`**。只有用户明确说“发布开源/同步 public”时才执行公开发布流程。
